@php
    if(Session::has('site_language')){
        $locale = Session::get('site_language', Config::get('app.locale'));
    }
    else{
                    ...
                    <ul class="nav">
                        <div class="dropdown">
                            <button type="button" {{ (empty($userCarts) or count($userCarts) < 1) ? 'disabled' : '' }} class="btn btn-transparent dropdown-toggle" id="navbarShopingCart" data-toggle="dropdown"
                                    aria-haspopup="true" aria-expanded="false" style="margin-top: -11px; border: 0">
                                ...
                                <ul class="languages">
                                    @if ( !empty($lang) )
                                        @for ($i=0;$i< count($lang);$i++)
                                            <li onclick="setLanguage('{{ $lang[$i]['code'] }}')" data-code={{$lang[$i]['code']}} class="{{ $locale == $lang[$i]['code'] ? 'active-lang' : '' }}"><span class="lang-flag"><img class="img-fluid" src="{{asset('language-flags/png100px').'/'.$lang[$i]['code'].'.png'}}" width="20px" alt=""></span>{{$lang[$i]['name']}}</li>
                                            {{-- <a href="{{ route('local',$lang[$i]['code']) }}" id={{$lang[$i]['id']}} data-code={{$lang[$i]['code']}} class="active selected {{ $locale == $lang[$i]['code'] ? 'active-lang' : '' }}"><span class="lang-flag"><img class="img-fluid" src="{{asset('language-flags/png100px').'/'.$lang[$i]['code'].'.png'}}" width="20px" alt=""></span>{{$lang[$i]['name']}}</a> --}}
                                        @endfor
                                    @endif
                                </ul>
                            </div>
                                ...
                                <div class="toooltip-content">
                                    <div class="user-dropdown">
                                        <ul>
                                            ...
                                            {{-- <li><a href=""{{ route('profile') }}""><i class="fal fa-user"></i> Profile</a></li> --}}
                                            <li><a href="#"><i class="fal fa-sliders-v"></i> Settings</a></li>
                                            <li><a href="{{ route('logout') }}"><i class="ri-shut-down-line align-middle me-1"></i> {{ __('Logout') }}</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        @endauth
                        @guest
                            {{-- <li><a href="https://ssoserver.dev2.kemedar.com/sso-login?redirect_uri={{ base64_encode(env('APP_URL')) }}"  class="text-white"><i class="fal fa-sign-in"></i></a></li> --}}
                            <li><a href="https://ssoserver.dev2.kemedar.com/sso-login?redirect_uri={{ base64_encode(config('app.url', '')) }}"  class="text-white"><i class="fal fa-sign-in"></i></a></li>
                    ...
                    <div class="toooltip" x-data="{ openl: false }">
                        ...
                        <div class="toooltip-content" x-show="openl" @click.outside="openl = false" style="display: none">
                            <ul class="languages">
                                @if($lang)
                                    @foreach($lang as $lan)
                                        <li data-code={{$lan['code']}} class="active"><span class="lang-flag"><img class="img-fluid" src="{{asset('language-flags/png100px').'/'.$lan['code'].'.png'}}" width="20px" alt=""></span>{{$lan['name']}}</li>
                                    @endforeach
