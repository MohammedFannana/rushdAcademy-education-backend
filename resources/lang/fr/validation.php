<?php

return array (
  'accepted' => 'L\'attribut doit être accepté.',
  'active_url' => 'L\'attribut n\'est pas une URL valide.',
  'after' => 'L\'attribut doit être une date après: date.',
  'after_or_equal' => 'L\'attribut doit être une date après ou égale à: Date.',
  'alpha' => 'L\'attribut ne peut contenir que des lettres.',
  'alpha_dash' => 'L\'attribut ne peut contenir que des lettres, des numéros, des tirets et des soulignements.',
  'alpha_num' => 'L\'attribut ne peut contenir que des lettres et des numéros.',
  'array' => 'L\'attribut doit être un tableau.',
  'before' => 'L\'attribut doit être une date avant: date.',
  'before_or_equal' => 'L\'attribut doit être une date avant ou égale à: Date.',
  'between' => 
  array (
    'numeric' => 'L\'attribut doit être entre: min et: max. (numérique)',
    'file' => 'L\'attribut doit être entre: min et: kilobytes max. (déposer)',
    'string' => 'L\'attribut doit être entre: min et: caractères max. (chaîne)',
    'array' => 'L\'attribut doit avoir entre: min et: éléments max. (tableau)',
  ),
  'boolean' => 'Le champ: Attribut doit être vrai ou faux.',
  'confirmed' => 'La confirmation: l\'attribut ne correspond pas.',
  'date' => 'L\'attribut n\'est pas une date valide.',
  'date_equals' => 'L\'attribut doit être une date égale à: Date.',
  'date_format' => 'L\'attribut ne correspond pas au format: format.',
  'different' => 'L\'attribut: Attribut et: Autre doit être différent.',
  'digits' => 'L\'attribut doit être: chiffres des chiffres.',
  'digits_between' => 'L\'attribut doit être entre: min et: chiffres max.',
  'dimensions' => 'L\'attribut a des dimensions d\'image non valides.',
  'distinct' => 'Le champ: Attribut a une valeur en double.',
  'email' => 'L\'attribut doit être une adresse e-mail valide.',
  'ends_with' => 'L\'attribut doit se terminer par l\'un des éléments suivants :: Valeurs.',
  'exists' => 'L\'attribut sélectionné n\'est pas valide.',
  'file' => 'L\'attribut doit être un fichier.',
  'filled' => 'Le champ: Attribut doit avoir une valeur.',
  'gt' => 
  array (
    'numeric' => 'L\'attribut doit être supérieur à: valeur. (numérique)',
    'file' => 'L\'attribut doit être supérieur à: KiloBytes de valeur. (déposer)',
    'string' => 'L\'attribut doit être supérieur à: les caractères de valeur. (chaîne)',
    'array' => 'L\'attribut doit avoir plus que: les éléments de valeur. (tableau)',
  ),
  'gte' => 
  array (
    'numeric' => 'L\'attribut doit être supérieur ou égal: valeur. (numérique)',
    'file' => 'L\'attribut doit être supérieur ou égal: des kilo-kilobytes. (déposer)',
    'string' => 'L\'attribut doit être supérieur ou égal: des caractères de valeur. (chaîne)',
    'array' => 'L\'attribut doit avoir: des éléments de valeur ou plus. (tableau)',
  ),
  'image' => 'L\'attribut doit être une image.',
  'in' => 'L\'attribut sélectionné n\'est pas valide.',
  'in_array' => 'Le champ: Attribut n\'existe pas dans: Autre.',
  'integer' => 'L\'attribut doit être un entier.',
  'ip' => 'L\'attribut doit être une adresse IP valide.',
  'ipv4' => 'L\'attribut doit être une adresse IPv4 valide.',
  'ipv6' => 'L\'attribut doit être une adresse IPv6 valide.',
  'json' => 'L\'attribut doit être une chaîne JSON valide.',
  'lt' => 
  array (
    'numeric' => 'L\'attribut doit être inférieur à: valeur. (numérique)',
    'file' => 'L\'attribut doit être inférieur à: Valeur kilobytes. (déposer)',
    'string' => 'L\'attribut doit être inférieur à: les caractères de valeur. (chaîne)',
    'array' => 'L\'attribut doit avoir moins de: éléments de valeur. (tableau)',
  ),
  'lte' => 
  array (
    'numeric' => 'L\'attribut doit être inférieur ou égal: valeur. (numérique)',
    'file' => 'L\'attribut doit être inférieur ou égal: des kilo-glus de valeur. (déposer)',
    'string' => 'L\'attribut doit être inférieur ou égal: des caractères de valeur. (chaîne)',
    'array' => 'L\'attribut ne doit pas avoir plus que: les éléments de valeur. (tableau)',
  ),
  'max' => 
  array (
    'numeric' => 'L\'attribut peut ne pas être supérieur à: Max. (numérique)',
    'file' => 'L\'attribut peut ne pas être plus grand que: kilo-kilo-kilo-great. (déposer)',
    'string' => 'L\'attribut peut ne pas être plus grand que: les caractères max. (chaîne)',
    'array' => 'L\'attribut peut ne pas avoir plus que: les éléments max. (tableau)',
  ),
  'mimes' => 'L\'attribut doit être un fichier de type :: valeurs.',
  'mimetypes' => 'L\'attribut doit être un fichier de type :: valeurs.',
  'min' => 
  array (
    'numeric' => 'L\'attribut doit être au moins: min. (numérique)',
    'file' => 'L\'attribut doit être au moins: kilo-kilo-kilo-. (déposer)',
    'string' => 'L\'attribut doit être au moins: les caractères min. (chaîne)',
    'array' => 'L\'attribut doit avoir au moins: les éléments min. (tableau)',
  ),
  'not_in' => 'L\'attribut sélectionné n\'est pas valide.',
  'not_regex' => 'Le format: attribut n\'est pas valide.',
  'numeric' => 'L\'attribut doit être un nombre.',
  'password' => 'Le mot de passe est incorrect.',
  'password_or_username' => 'Le mot de passe ou le nom de l\'utilisateur est incorrect.',
  'present' => 'Le champ: Attribut doit être présent.',
  'regex' => 'Le format: attribut n\'est pas valide.',
  'required' => 'Le champ: Attribut est requis.',
  'required_if' => 'Le champ: Attribut est requis lorsque: l\'autre est: valeur.',
  'required_unless' => 'Le champ: Attribut est requis à moins que: l\'autre est dans: VALEURS.',
  'required_with' => 'Le champ: Attribut est requis lorsque: des valeurs sont présentes.',
  'required_with_all' => 'Le champ: Attribut est requis lorsque: des valeurs sont présentes.',
  'required_without' => 'Le champ: Attribut est requis lorsque: les valeurs ne sont pas présentes.',
  'required_without_all' => 'Le champ: Attribut est requis lorsqu\'aucune des valeurs n\'est présente.',
  'same' => 'L\'attribut: Attribut et: Autre doit correspondre.',
  'size' => 
  array (
    'numeric' => 'L\'attribut doit être: taille. (numérique)',
    'file' => 'L\'attribut doit être: Taille kiloBytes. (déposer)',
    'string' => 'L\'attribut doit être: des caractères de taille. (chaîne)',
    'array' => 'L\'attribut doit contenir: Éléments de taille. (tableau)',
  ),
  'starts_with' => 'L\'attribut doit commencer par l\'un des éléments suivants :: Valeurs.',
  'string' => 'L\'attribut doit être une chaîne.',
  'timezone' => 'L\'attribut doit être une zone valide.',
  'unique' => 'L\'attribut a déjà été pris.',
  'uploaded' => 'L\'attribut n\'a pas réussi à télécharger.',
  'url' => 'Le format: attribut n\'est pas valide.',
  'uuid' => 'L\'attribut doit être un UUID valide.',
  'captcha' => 'Captcha incorrecte...',
);
