...
function linkSSO($link) {
    if(($link != '' || $link != '#') &&  getDomain($link) != getDomain(config('app.url')) && Auth::check()) {
        $token = Session::get('auto_login') ? Session::get('auto_login') : '';
...
// function returnSSOLinkWithLang($type){
//     $lang_redirect = session('locale') ? session('locale') : 'en';
//     $sso_login = 'https://sso.kemedar.com/sso-login?lang='.$lang_redirect.'&redirect_uri=' . base64_encode(request()->fullUrl().'/panel');
//     $sso_register = 'https://sso.kemedar.com/sso-register?lang='.$lang_redirect.'&redirect_uri='. base64_encode(request()->fullUrl());
//     if($type == 'login'){
//         return $sso_login;
//     }
//     else
...
function returnSSOLinkWithLang($type){
    $lang_redirect = session('locale') ? session('locale') : 'en';
    $sso_login = 'https://gaza-academy.com/login?lang='.$lang_redirect.'&redirect_uri=' . base64_encode(request()->fullUrl().'/panel');
    $sso_register = 'https://gaza-academy.com/register?lang='.$lang_redirect.'&redirect_uri='. base64_encode(request()->fullUrl());
    if($type == 'login'){
        return $sso_login;
    }
    else
